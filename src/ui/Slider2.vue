<template>
  <input-field
    ...
    @blur="onBlur"
  >
    <div ref="slider" class="slider" :tabindex="disabled ? -1 : 0">
...
export default {
  ...
  methods: {
    ...
    trasformValue (v) {
      return this.transform ? this.transform(v) : v
    },
    // focus () {
    ...
    dragStart (e) {
      ...
      const onMove = evt => {
        ...
        if (!this.lazy) {
          this.$emit('input', this.trasformValue(value))
        }
      }
      const onEnd = evt => {
        ...
        evt.stopPropagation()
        evt.preventDefault()
        const value = this.trasformValue(this.dragValue) // or comupte value from coords?
        this.dragging = false
        this.dragValue = null
        if (!drag) {
          return
    ...
    valueOnClick (e) {
      ...
      let value = this.range.min + (offset / trackBounds.width) * (this.range.max - this.range.min)
      value = this.trasformValue(value)
      this.$emit('input', value)
      this.$emit('change', value)
    }
  }
